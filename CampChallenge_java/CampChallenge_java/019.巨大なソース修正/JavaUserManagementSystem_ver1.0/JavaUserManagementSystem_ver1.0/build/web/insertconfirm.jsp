...
<html>
    ...
    <% }else{ %>
        <h1>入力が不完全です</h1>
    <% if (hs.getAttribute("name").equals("")){%>『名前』<% } %>
    <% if (hs.getAttribute("year").equals("") ||
           hs.getAttribute("month").equals("") ||
           hs.getAttribute("day").equals("")){%>『生年月日』<% } %>
    <% if (hs.getAttribute("type").equals("")){%>『種別』<% } %>
    <% if (hs.getAttribute("tell").equals("")){%>『電話番号』<% } %>
    <% if (hs.getAttribute("comment").equals("")){%>『自己紹介文』<% } %>
    を入力してください。
